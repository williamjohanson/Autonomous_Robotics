...
def get_data():
    ...
    # Split into columns
    _, time, range_, velocity_command, _, _, IR3, _, _, _ = data.T
    ...
    return time, range_, velocity_command, IR3
...
#def sonar_filter(time, range_, disp_command, IR3):
...
#def linear_least_squares(fil_range_sonar1, fil_sonar1, fil_disp_command_sonar1):
...
def model_nonlinear_least_squares_fit(r, v, iterations=5):
    ...
    # Calculate Jacobians for current estimate of parameters.
        ...
        deltak, res, rank, s = lstsq(A, v - model_r(r, k))
        k += deltak
        print(k)
    return k
...
def approximates(fil_sonar1, fil_disp_command_sonar1, theta_1, theta_2):
...
def plotting(time, fil_time_sonar1, range_, x, x_est,f_v_sonar1_plot, f_v_sonar1 , x_range): 
    """ Plot some results. """
    plt.figure()
    plt.subplot(141)
    plt.plot(np.array(time), np.array(range_), '.', alpha=0.2, color='b')
    plt.plot(np.array(time), np.array(x), '.', alpha=0.2, color='y')
    plt.plot(np.array(time), np.array(x_est), '.', alpha=0.2, color='g')
...
def main():
    ...
    time, range_, velocity_command, IR3 = get_data()

    disp_command = vel_to_disp(time, velocity_command, IR3)

    #fil_range_sonar1, fil_time_sonar1,  fil_sonar1, fil_disp_command_sonar1,  = sonar_filter(time, range_, disp_command, IR3)

    k = model_nonlinear_least_squares_fit(Z_vector, x_raw)
    h_x_raw = model_x(x_raw, k)

    #k = linear_least_squares(range_, IR3, velocity_command)

    print(k)

    x, x_est = approximates(h_x_raw, , theta_1)      

    f_v_sonar1_plot, f_v_sonar2_plot, f_v_sonar1, f_v_sonar2, x_range = error_PDF(x, x2, x_est, x2_est)

    plotting(time, fil_time_sonar1, range_, x, x_est,f_v_sonar1_plot, f_v_sonar1 , x_range)
